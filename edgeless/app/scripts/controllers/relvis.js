...
var edgelessApp = angular.module('edgelessApp');


edgelessApp.controller('relvisCtrl', function ($scope, $interval, $window) {
		...
		return $interval(function() {
			...
			if (nodes[i] && nodes[i].stability) {
				...
				for (var j =0; j<$scope.numTargets; j++) {
					...
					var targetLine={
						"style": {
							"opacity": nodes[i].stability/$scope.maxStability-0.5,
						},
	...
	var resetGrid = function() {
		var maxWidth = Math.min(windowWidth,1170)*0.74;
...
edgelessApp.controller('stabilityGraphController', function ($scope, $interval) {
	var vals;
